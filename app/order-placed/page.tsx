...
import { useBookingPlaced } from '@/hooks/api/useOrder';
...
function OrderPlacedContent() {
  ...
  const { data: booking, isLoading, error } = useBookingPlaced(sessionId);
  ...
  const formattedDate = useMemo(() => {
    if (!booking?.pickup_date) return 'N/A';
    return new Date(booking.pickup_date).toLocaleDateString('en-GB', {
      day: '2-digit',
      month: 'short',
      year: 'numeric',
    });
      ...
      <div className="w-full bg-light-background flex flex-col min-h-screen">
        ...
        <div className="max-w-5xl mx-auto py-16 lg:py-24 w-full flex items-center justify-center flex-col gap-6 lg:gap-12 text-center p-3">
          ...
          <div className="w-full grid md:grid-cols-3 lg:gap-5">
            <div className="lg:col-span-2 w-full bg-white max-lg:rounded-b-xl max-lg:order-2 lg:rounded-l-xl border border-text-gray border-2 shadow-lg py-6 px-4 gap-8 flex flex-col">
  ...
  if (error) {
    return (
      <div className="w-full bg-light-background flex flex-col min-h-screen">
        <div className="h-24 w-full bg-heading-black"></div>
        <div className="max-w-5xl mx-auto py-16 lg:py-24 w-full flex items-center justify-center flex-col gap-6 text-center p-3">
          <div className="text-red-500 text-4xl mb-4">⚠️</div>
          <h2 className="text-2xl font-bold text-heading-black mb-2">Unable to Load Booking</h2>
          <p className="text-text-gray mb-6">{error instanceof Error ? error.message : 'An error occurred'}</p>
          <button
            onClick={() => window.location.reload()}
            className="bg-heading-black text-white px-6 py-2 rounded-lg hover:opacity-90 transition-opacity"
          >
            Try Again
          </button>
        </div>
      ...
      <div className="max-w-5xl mx-auto py-16 lg:py-24 w-full flex items-center justify-center flex-col gap-6 lg:gap-12 text-center p-3">
        <div className="w-full flex items-center justify-center flex-col gap-3 lg:gap-5">
          ...
          <div className="text-heading-black text-lg font-medium">
            Great choice{booking.name ? `, ${booking.name}` : ''}!
          </div>
        ...
        <div className="w-full grid md:grid-cols-3 lg:gap-5">
          <div className="lg:col-span-2 w-full bg-white max-lg:rounded-b-xl max-lg:order-2 lg:rounded-l-xl border border-text-gray border-2 shadow-lg py-6 px-4 gap-8 flex flex-col justify-center text-start">
              ...
              <div className="flex flex-col gap-3 w-full">
                {booking.pickup_location && (
                  <div className="flex items-start gap-3 md:gap-5">
                    <div className="max-lg:text-sm text-heading-black">{booking.pickup_location}</div>
                  </div>
                )}
                {booking.stops && booking.stops.length > 0 && booking.stops.map((stop, index) => (
                  <div key={index} className="flex items-start gap-3 md:gap-5">
                    <div className="max-lg:text-sm text-heading-black">{stop}</div>
                  </div>
                ))}
                {booking.category === 'hourly' && booking.duration && (
                  <div className="flex items-start gap-3 md:gap-5">
                    <div className="max-lg:text-sm text-heading-black">{booking.duration} Hours</div>
                  </div>
                )}
                {booking.category !== 'hourly' && booking.dropoff_location && (
                  <div className="flex items-start gap-3 md:gap-5">
                    <div className="max-lg:text-sm text-heading-black">{booking.dropoff_location}</div>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="flex flex-col gap-1">
              ...
              <div className="text-heading-black font-semibold">
                {formattedDate} {booking.pickup_time || ''}
              </div>
            </div>
          </div>
          ...
          {booking.fleet && (
            <div className="max-lg:rounded-t-xl lg:rounded-r-xl border-2 border border-text-gray shadow-lg p-4 gap-1 flex flex-col items-center justify-center bg-light-background max-lg:order-1">
              ...
              <div className="font-bold text-heading-black text-lg">{booking.fleet.name}</div>
              {booking.fleet.cars && (
                <div className="text-text-gray text-sm text-center">{booking.fleet.cars}</div>
              )}
            </div>
          )}
        </div>
