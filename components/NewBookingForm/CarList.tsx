  ...
  return (
    <div className="w-full flex flex-col gap-5 ">
      {fleets.map((item) => {
        ...
        if (category === "hourly") {
          price = Number(
            ((hours >= 2 ? hours : 2) * Number(item.hourly)).toFixed(2)
          );
        } else if (category === "trips") {
          price = distance * item.price;
          // if (distance < 10) {
          //     price = item.under10
          // } else if (distance < 20) {
          //     price = item.under20
        ...
        if (price > item.minimumFare) {
          price = item.minimumFare;
        } else {
          price = Number(price.toFixed(2));
        }
        let returnPrice = Number(
          (price * 2 - ((price * 2) / 100) * 7).toFixed(2)
        );
