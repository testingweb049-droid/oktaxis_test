...
export default function Header() {
  ...
  const shouldHaveBlackHeader =
    ["/contact", "/terms", "/cookies", "/faqs", "/privacy", "/driver"].includes(pathname) ||
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (openDropdown && !target.closest(".services-dropdown-container") && width >= 786) {
        console.log("work")
        setOpenDropdown(null)
      }
    }
  ...
  const toggleDropdown = (id: string) => {
    console.log()
    setOpenDropdown((prev) => { console.log("prev : ", prev); console.log("id: ", id); return prev === id ? null : id })
  }
  ...
  return (
    <>
        ...
        <div className="max-w-screen-2xl mx-auto px-3 md:px-6 relative">
          <div className="flex items-center justify-between">
            ...
            <nav className="hidden items-center space-x-1 md:flex">
              {navLinks.map((link) => (
                <div key={link.id} className="relative services-dropdown-container">
                  {link.submenu ? (
                    <>
                      <button
      ...
      <div
        className={`mobile-menu-container fixed top-0 left-0 right-0 z-40 md:hidden transition-all duration-300 ease-in-out ${isOpen ? "opacity-100 translate-y-0 pointer-events-auto" : "opacity-0 -translate-y-full pointer-events-none"
          }`}
        style={{
      ...
      >
        <div className="bg-black/95 backdrop-blur-sm shadow-lg border-t border-gray-700">
          <nav className="px-4 py-4 space-y-1 max-h-[calc(100vh-80px)] overflow-y-auto">
            {navLinks.map((link) => (
              <div key={link.id} className="border-b border-gray-700 last:border-b-0 pb-1 last:pb-0">
                {link.submenu ? (
                  <div>
                    <button
                      onClick={() => {
                      ...
                      className={`flex items-center justify-between w-full text-left py-2 text-lg font-medium transition-colors ${pathname.startsWith("/services") ? "text-amber-500" : "text-white hover:text-amber-500"
                        }`}
                      aria-expanded={openDropdown === link.id}
                      aria-haspopup="true"
                    >
                      ...
                      <div className="pl-4 pt-2 space-y-2">
                        {link.sublinks?.map((sublink, index) => (
                          <Link
                            key={sublink.id}
                            href={sublink.path}
                            className={`block py-2 text-base transition-all duration-200 ${pathname === sublink.path ? "text-amber-500" : "text-gray-300 hover:text-amber-500"
                              }`}
                            style={{
                              transitionDelay: openDropdown === link.id ? `${index * 50}ms` : "0ms",
                            }}
                  ...
                  <Link
                    href={link.path || "#"}
                    className={`block py-2 text-lg font-medium transition-colors ${pathname === link.path ? "text-amber-500" : "text-white hover:text-amber-500"
                      }`}
                  // onClick={closeAllDropdowns}
                  >
                    {link.title}
